...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Ein Regler zur Einstellung der Temperatur. Dieser hat eine Einrastfunktion, dass man nicht zwischen zwei Modi sein kann.
    \end{itemize}
...
\begin{enumerate}[label=\textbf{R-1.\arabic*}, ref=R-1.\arabic*, itemsep=0pt, topsep=0pt, font=\bfseries]
    ...
    \end{itemize}
    \item \label{req:1.13} Das System muss die Heizfunktion automatisch deaktivieren, wenn ein Timer abgelaufen ist. -> macht gar kein sinn weil drehregler
\end{enumerate}
...
\subsection{Ziele der Iteration}
Ziel dieser Iteration war die Entwicklung und Integration einer grundlegenden Logging-Struktur zur Protokollierung von Systemverhalten und potenziellen Fehlern. Die Funktionalität wurde exemplarisch in bestehenden Komponenten eingebaut, sodass sie bereits zur Laufzeit relevante Informationen ausgeben können. Zudem wurden die Voraussetzungen geschaffen, um zukünftige Module ohne zusätzlichen Implementierungsaufwand in das Logging-System einzubinden.

Folgende Anforderung wurde in dieser Iteration (teilweise) umgesetzt:

\begin{itemize}
    \item \textbf{\ref{req:3.3}}: Logging von Informationen und Fehlern (teilweise umgesetzt; derzeit beschränkt auf bereits entwickelte Funktionalitäten)
\end{itemize}
...
\textbf{Requirement} & \textbf{Beschreibung} & \textbf{Klasse/Methode} & \textbf{Testfälle} & \textbf{Status} \\
\hline
\ref{req:3.3} & Logging von Informationsmeldungen & \texttt{Logger.Info(string)} & manueller Test & OK \\
\hline
\end{tabular}

Die Logging-Funktion wurde durch einen manuellen Tests überprüft. Dabei wurde dem \texttt{TemperatureRotaryController} ein definierter Winkelwert zugewiesen. Anschließend wurde mittels der Konsolenausgabe überprüft, ob die Loggerausgaben korrekt generiert wurden und alle relevanten Informationen enthalten waren. Diese Vorgehensweise ermöglichte eine effektive Analyse des Systemverhaltens zur Laufzeit und bestätigte die Funktionalität der implementierten Logging-Infrastruktur.

\subsection{Abweichungen vom Feinentwurf}

\begin{itemize}
    \item Zusätzlich zum ursprünglichen Entwurf wurde für die Display-Komponente eine Proxy-Klasse eingeführt, um gezielt Logging-Funktionalitäten zu integrieren.
    \item Der \texttt{TemperatureRotaryController} wurde um eine dedizierte Testmethode erweitert, mit der der Winkelwert direkt aus Testfällen heraus gesetzt werden kann. Diese Methode ist mit einem \texttt{\#if DEBUG}-Block versehen und wird im Release-Build ausgeschlossen.
\end{itemize}
...
\subsection{Ziele der Iteration}
Das Ziel dieser Iteration ist die Implementierung einer Statemachine des Ofens und von verschiedenen Strategien die je nach ausgewähltem Modus unterschiedlich agieren. Dies ermöglicht erstmals eine variable Nutzung des Backofens.
...
\begin{itemize}
    ...
    \item \textbf{\ref{req:1.5}}: Folgende Betriebsmodi müssen wählbar sein: Ober-/Unterhitze, Oberhitze, Unterhitze, Grillfunktion, Umluft, Heißluft.
    ...
    \begin{itemize}
        ...
        \item Heißluft: Ringheizkörper hinten + Ventilator
    \end{itemize}
    \item \textbf{\ref{req:1.11}}: Im Grillmodus muss das System die eingestellte Temperatur intern in vier Grillstufen (240–300°C) umwandeln.
    \item \textbf{\ref{req:3.3}}: Logging von Informationen und Fehlern (teilweise umgesetzt; derzeit beschränkt auf bereits entwickelte Funktionalitäten)
    \item Todo requirement 2.3
\end{itemize}
...
\textbf{Requirement} & \textbf{Beschreibung} & \textbf{Klasse/Methode} & \textbf{Testfälle} & \textbf{Status} \\
\hline
\ref{req:1.4} & Auswählen eines Betriebsmodus über einen Drehregler & \texttt{ModeRotaryController.ReadInput()} & TC-3-2 & OK \\
\hline
\ref{req:1.5} & Auswahl an Betriebsmodi & \texttt{BaseModeStrategy.Run()} & TC-3-2 & OK \\
\hline
\ref{req:1.6} & Zuordnung der Heizaggregate zu den Betriebsmodi & \texttt{BaseModeStrategy.\_thermalControllers} & TC-4-1, TC-4-2, TC-4-3, TC-4-5, TC-4-6, TC-4-7, TC-4-8 & OK \\
\hline
\ref{req:1.11} & Stufen der Grillfunktion & \texttt{GrillMode.CalculateStepTemperature()} & TC-4-4 & OK \\
\hline
\ref{req:3.3} & Logging von Informationsmeldungen & \texttt{Logger.Info(string)} & manueller Test & OK \\
\hline
\ref{req:2.3} & Durchlaufzeit von unter 1 Sekunde & \texttt{todo} & TC-0-1 & OK \\
\hline
\end{tabular}


Die Logging-Funktion wurde durch einen manuellen Tests überprüft. Dabei wurde dem \texttt{TemperatureRotaryController} ein definierter Winkelwert zugewiesen, ebenso die der \texttt{ModeRotaryController}. Anschließend wurde mittels der Konsolenausgabe überprüft, ob die Loggerausgaben korrekt generiert wurden und alle relevanten Informationen enthalten waren. Diese Vorgehensweise ermöglichte eine effektive Analyse des Systemverhaltens zur Laufzeit und bestätigte die Funktionalität der implementierten Logging-Infrastruktur.

\subsection{Abweichungen vom Feinentwurf}

\begin{itemize}
    \item Es wurde eine weitere Proxyklasse für die Statemachine im OvenControllerModule implementiert um auch das verhalten dieser zu loggen. Daher benötigt auch der OvenController keine Loggingfunktionalität mehr.
    \item Der OvenState wurde gänzlich weg gelassen, da er bei der Implementierung als irrelevant empfunden wurde. CurrentState im OvenController wird direkt über das State-Objekt geregelt.
    \item Der Übergang von einem State in einen anderen wurde in IState eine eigene Methode CheckStateTransition() ausgelagert. Diese wird in jeder Unterklasse implementiert.
    \item Die States bekommen wichtige Instanzvariablen, die sie für ihre Funktionalität benötigen.
    \item Der OffState wurde weg gelassen, da er bei der Implementierung als irrelevant empfunden wurde.
    \item Die Vererbungshierarchie im ModeControlModule wurde um die BaseModeStrategy erweitert. Diese übernimmt Algorithmen, die in jedem State vorhanden wäre. Ebenso bekommt diese eine Liste mit IThermalController die die jeweilige Strategie steuert. 
\end{itemize}
...
\subsection{Ziele der Iteration}
Diese Iteration ist dafür da Sicherheitsüberprüfungen in dem Backofen einzubauen. Diese setzen die Requirements \ref{req:2.1} bis \ref{req:2.3} ab. Außerdem wurde ein Timer implementiert.

\begin{itemize}
    \item \textbf{\ref{req:2.1}}: Das System muss den Heizbetrieb automatisch abschalten, wenn die Temperatur 320°C überschreitet.
    \item \textbf{\ref{req:2.2}}: Das System darf den Heizbetrieb nicht aktivieren, wenn die Backofentür geöffnet ist.
    ...
    \item \textbf{\ref{req:3.3}}: Das System muss alle sicherheitsrelevanten Zustandsänderungen und Fehlerereignisse mit Zeitstempel in einer Logdatei protokollieren.
\end{itemize}
...
\textbf{Requirement} & \textbf{Beschreibung} & \textbf{Klasse/Methode} & \textbf{Testfälle} & \textbf{Status} \\
\hline
\ref{req:2.1} & Überhitzcheck & \texttt{OverheatRule.Check()} & TC-8-6 & OK \\
\hline
\ref{req:2.2} & Türcheck & \texttt{DoorOpenRule.Check()} & TC-8-1, TC-8-2 & OK \\
\hline
\ref{req:2.3} & Hitzeaggregatcheck & \texttt{HeaterFailureRule.Check()} & TC-8-3, TC-8-4, TC-8-5 & OK \\
\hline
\ref{req:3.3} & Logging von Informationsmeldungen & \texttt{Logger.Info(string)} & manueller Test & OK \\
\hline
\end{tabular}
...
Die Logging-Funktion wurde durch einen manuellen Tests überprüft. Dabei wurde dem \texttt{TemperatureRotaryController} ein definierter Winkelwert zugewiesen, ebenso die der \texttt{ModeRotaryController}. Es wurde Werte ausgewählt, um gezielt die Sicherheitschecks zu verletzen. Anschließend wurde mittels der Konsolenausgabe überprüft, ob die Loggerausgaben korrekt generiert wurden und alle relevanten Informationen enthalten waren. Diese Vorgehensweise ermöglichte eine effektive Analyse des Systemverhaltens zur Laufzeit und bestätigte die Funktionalität der implementierten Logging-Infrastruktur.

\subsection{Abweichungen vom Feinentwurf}

\begin{itemize}
    \item Es wurde sich dagegen entschieden nach Ablauf des Timers den Backofen aus zu schalten, da man sonst einen Motor im Drehregler benötigen würde um diesen auf den Ursprung zurück zu stellen oder es kommt zu inkonsistenz. Ebenso ist es bei den meisten herkömmlichen Backöfen nicht der Fall, dass ein integrierter Timer diesen Ausschalten kann.
    \item Es wurde eine weitere Proxyklasse für die Sicherheitsregeln implementiert um das Loggen außerhalb des Produktivcodes zu haben.
    \item Die HeaterFailureRule bekommt ein int-Array anstatt eines double-Arrays für die letzten Temperaturen.
\end{itemize}
...
\begin{itemize}
    ...
    \item Der OvenController bekommt eine Methode um den aktuellen State zurückzugeben, ebenso wie der StateProxy.
\end{itemize}
